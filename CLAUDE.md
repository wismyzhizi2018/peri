...
**TUI 消息渲染**（`peri-tui`）：所有消息更新通过统一 `RebuildAll` 路径触发（无增量更新）。`MessagePipeline`（`message_pipeline.rs`）维护规范状态，`build_tail_vms()` 构建尾部 VMs，`messages_to_view_models()` 是唯一转换入口。流式文本通过 16ms 间隔 + 自适应分块策略触发 RebuildAll，非流式事件立即触发。独立 `RenderThread` 处理渲染，通过 `RenderCache(RwLock)` 与 UI 线程同步。
...
- **SubAgent 系统提示词**：`language`/`date` 已通过 `frozen_language_for_sub`/`frozen_date_for_sub` 正确冻结。**[TRAP]** 但 `PromptFeatures::detect()` 仍每轮重新读取 `YOLO_MODE`（影响 HITL 段落注入），`is_git_repo` 也每轮重新检查——两者未随 frozen 数据传递，可能导致 SubAgent 与 Main Agent 在同一会话中行为不一致。所有 frozen 字段必须通过 `AcpAgentConfig` 传递到 SubAgent builder。（详见 spec/global/domains/system-prompt.md#issue_2026-05-27-language-injection-subagent-drift-cache-isolation）
...
`.claude/agents/` 下定义，支持两种格式：扁平 `{agent_id}.md` 和嵌套 `{agent_id}/agent.md`。`tools` 为空继承父工具（排除 Agent 防递归），有值仅保留允许列表，`disallowedTools` 额外排除。关键 frontmatter 字段：`model`（模型选择）、`maxTurns`（最大轮数）、`background`（后台运行）、`isolation`（worktree 隔离）、`skills`（启动加载）、`permissionMode`、`mcpServers`、`hooks`；完整定义见 `ClaudeAgentFrontmatter`。插件 agent 通过 `scan_agents_with_extra_dirs` 追加搜索路径。内置 agents（explore/general-purpose/plan/verification）编译期嵌入，同名被项目级覆盖。
...
**[TRAP]** 并发 SubAgent 场景：事件路由必须用 `source_agent_id` 精确匹配而非位置堆栈；流式循环必须 `tokio::select!` 竞争取消令牌防止 Ctrl+C 死锁；事件通道：主 executor 和 `bg_event_tx` 均用 `unbounded_channel()`。Background Fork 使用 `CancelPolicy::Independent`，父 Agent Ctrl+C 不传播，只能通过 `session/cancel_all_agents` 取消。（详见 spec/global/domains/agent.md#issue_2026-05-16-concurrent-subagent-tool-call-routing-and-background，spec/global/domains/tui.md#issue_2026-05-15-concurrent-subagent-display-delay）

**[TRAP]** 同步 SubAgent 取消传播：父 Agent 的 cancel token 通过 `CancelPolicy::Cascade → child_token()` 传播到同步 SubAgent 执行上下文，否则 Ctrl+C 无法中断 SubAgent。（详见 spec/global/domains/agent.md#issue_2026-05-25-ctrl-c-cannot-interrupt-sync-subagent）
...
**主布局**（`ui/main_ui/mod.rs`）：单 Session 垂直切分（Sticky Header → Messages → Attachment Bar → Panel Area → Input → Status Bar → BG Agent Bar）。高度优先级：Status Bar 固定 3 行 → Input 动态（3~40% 屏幕）→ 面板（60-75% 屏幕）→ 其余分配给消息区。

**Welcome Card**（`ui/welcome.rs`）：`view_messages.is_empty()` 时替代消息区。宽屏（≥50 列）显示 6 行 ASCII Art "PERI"（ansi_shadow 字体），窄屏单行文字。内容：3 个功能要点 + `/model` `/history` `/help` `/agents` 命令提示 + Provider/Model 信息 + 可用 Skills 数量。未配置 Provider 时显示 `/login` 引导。居中布局，Accent/Warning/Muted 色系。

**Sticky Header**（`ui/main_ui/sticky_header.rs`）：`max_scroll > 0` 时在消息区顶部固定显示最后一条 Human 消息折叠摘要。宽度自适应估算行数（`char_count.div_ceil(width)`，clamp 1-3 行）。优先空白处断行，超长加 `…` 截断。背景 `USER_BG` + `❯` 前缀。

**Attachment Bar**（`ui/main_ui/attachment.rs`）：有 `pending_attachments` 时固定 3 行，位于 Input 正上方。显示 `[img {label} {size}KB]` 标签列表 + Del 删除提示。Accent 色边框。

**Input Area**（`edit_utils.rs` + `main_ui/mod.rs`）：基于 `tui_textarea::TextArea` 封装。灰色边框 + 左侧 `❯` 提示符（2 列 padding）。高度动态：`(line_count + 2).min(area.height * 2/5).max(3)`。聚焦后台 Agent 时显示彩色边框 + Agent 名称标签；弹窗激活时边框变暗。

**Hints 浮层**（`ui/main_ui/popups/hints.rs`）：输入以 `/` 开头时激活，从 `CommandRegistry` 匹配命令 + Skills + Agent 命令。排序：前缀匹配优先 → 类型优先级（命令 > Skills > Agent 命令）→ 字母序。最多 10 行 + 滚动。`❯` 光标选择 + 匹配前缀高亮。位置在输入框上方。

**@提及弹窗**（`app/at_mention/mod.rs`）：输入 `@` 后检测触发（需前面是空白），独立线程异步文件搜索（200ms 节流），在输入框上方显示候选文件列表。

**弹窗系统**（`ui/main_ui/popups/`）：统一通过 `InteractionPrompt` 枚举互斥管理。4 种弹窗：
- **HITL 审批**（`hitl.rs`）：`Approval(HitlBatchPrompt)`，批量工具调用逐个审批（↑↓ 移动 / Space 切换 / Enter 确认）
- **AskUser 问答**（`ask_user.rs`）：`Questions(AskUserBatchPrompt)`，Tab 栏切换问题 + 选项列表 + 自定义输入
- **OAuth 授权**（`oauth.rs`）：显示 URL + Ctrl+O 打开浏览器 + 回调输入
- **Setup Wizard**（`setup_wizard.rs`）：首次使用配置向导，4 步（Choose Provider → Language → Form 配置 → Done 确认）。Form 阶段支持 Browse（Provider 列表 + API Key 脱敏 `前4…后4`）/ Edit（字段级编辑 + 联通性测试 + 模型别名映射）

**面板系统**（12 种 `PanelKind`，分 Session/Global 作用域）：

| 面板 | 文件 | 作用域 | 内容 |
|------|------|--------|------|
| ModelPanel | `model_panel.rs` | Session | 模型选择（Opus/Sonnet/Haiku）+ MaxTokens/Effort/1M Context |
| LoginPanel | `login_panel/` | Session | Provider 管理（Browse/Edit/New/Delete 4 模式） |
| ConfigPanel | `config_panel.rs` | Session | AutoCompact/语言/Diff/Persona/Tone/Proactiveness/Threshold |
| AgentPanel | `agent_panel.rs` | Session | Agent 选择列表（含"无 Agent"选项） |
| HooksPanel | `hooks_panel.rs` | Session | Hooks 只读列表（事件类型 + 详情） |
| ThreadBrowser | `thread/browser.rs` | Session | 会话浏览器（搜索 + 列表 + 相对时间/大小） |
| McpPanel | `mcp_panel/` | Global | MCP 服务器列表（按来源分组）+ 详情（工具列表 + 操作菜单） |
| PluginPanel | `plugin_panel/` | Global | 插件管理（已安装/发现市场/详情/添加市场） |
| CronPanel | `cron_state.rs` | Global | 定时任务列表（表达式/下次触发/prompt） |
| TasksPanel | `tasks_panel.rs` | Global | Agent Threads + Cron Tasks（Tab 切换）+ Thread 详情 |
| StatusPanel | `status_panel.rs` | Global | 会话统计（Cost/Context Tab + 柱状图） |
| MemoryPanel | `memory_panel.rs` | Global | Memory 文件列表（路径 + 存在状态） |

面板互斥组（`MutexGroup`）：Settings（Model/Login/Config）、Agent（Agent/Hooks）、Tools（MCP/Plugin/Cron/Tasks）、Info（Status/Memory）、Thread（ThreadBrowser 独占）。

**BG Agent Bar**（`ui/main_ui/bg_agent_bar.rs`）：`background_agents.len() > 0` 时在 Status Bar 下方显示。高度 `1 + count.min(4) + 1`。8 色循环（Cyan/Magenta/Yellow/Green/Blue/Red/Orange/Purple），每项 `● {name} {steps}calls {elapsed}`。支持 `bg_bar_cursor` 聚焦切换（REVERSED 高亮），溢出显示 `…+N`。

**消息区**（`ui/main_ui/message_area.rs`）：Welcome Card 或消息列表 + 滚动条 + spinner。视口裁剪渲染（`viewport_clip`）只渲染可见行。`MessageViewModel` 7 种变体：`UserBubble` / `AssistantBubble`（含 Text/Reasoning/ToolUse） / `ToolBlock` / `SystemNote` / `CacheWarning` / `ToolCallGroup` / `SubAgentGroup`。
...
- **Status Bar**（`ui/main_ui/status_bar.rs`）：双行布局，每行用 `render_truncated_line()` 自适应截断。
  - **第一行**（`render_first_row`）：权限模式标签（非 Default 时显示 Don't Ask/Accept Edit/Auto Mode/Bypass，切换后 3 秒闪烁高亮）→ 工作目录名 → 模型名（切换后 3 秒闪烁）→ CPU%（<30 绿/30-70 黄/>70 红）→ MEM MB（<512 绿/512-1G 黄/>1G 红）→ 上下文使用率（`{pct}% {total}k/M`，≥70 黄/≥85 红）
  - **第二行**（`render_second_row`）：左侧瞬时状态（复制成功提示 3 秒 / 后台 agent 数量 / Agent 面板当前 agent 名 / LLM 重试进度 attempt/max+delay+error / MCP 初始化进度 connecting/ready/failed / LSP 诊断 errors+warnings）→ 右侧快捷键 hints（`format_hints()` 统一渲染 `(key, desc)` 对列表，key 加粗灰色 + desc 灰色）。Hints 按 `interaction_prompt` 状态动态切换：OAuth → Questions（Tab/↑↓/Space/Enter）→ Approval（↑↓/Space/Enter）→ 默认（面板 `status_bar_hints()` > 退出确认 > 单会话（/ / Shift+Enter / Ctrl+T））
  - 新增 Status Bar 元素时注意：瞬时提示用 `Instant` + Duration 控制消失；颜色分级统一用 `theme::ERROR`/`WARNING`/`SAGE`；面板 hints 通过 `PanelComponent::status_bar_hints()` trait 方法注入，不硬编码
...
- **`CommandRegistry::dispatch` 借用限制 [TRAP]**：`&self` + `&mut App` 冲突，当前用 `std::mem::take` + put-back 解决。在 dispatch 期间不可改变 `app.session_mgr` 的 session 实例。
...
- **跨平台 spawn [TRAP]**：所有子进程 spawn 必须通过平台感知的统一 wrapper（`shell_command()`），Windows 用 `cmd /C`、Unix 用 `bash -c`。三处调用点（MCP transport、Hooks executor、Bash 工具）已统一，新增 spawn 时必须复用。（详见 spec/global/domains/mcp.md#issue_2026-05-27-cross-platform-spawn-wrapper）
