...
**主布局**（`ui/main_ui/mod.rs`）：单 Session 垂直切分（Sticky Header → Messages → Attachment Bar → Panel Area → Input → Status Bar → BG Agent Bar）。多 Session 水平分栏（各列独立垂直布局，底部共享 Status Bar + BG Agent Bar，聚焦列 `theme::ACCENT` 边框、非聚焦 `theme::BORDER_DIM`）。高度优先级：Status Bar 固定 3 行 → Input 动态（3~40% 屏幕）→ 面板（60-75% 屏幕）→ 其余分配给消息区。
