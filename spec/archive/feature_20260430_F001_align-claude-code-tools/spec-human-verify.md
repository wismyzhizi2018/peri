...
- `git stash && cargo test -p peri-tui --lib -- test_subagent_group_basic 2>&1 | grep FAILED; git stash pop`
...
- `for f in write.rs edit.rs glob.rs read.rs grep.rs todo.rs ask_user_tool.rs; do echo "=== $f ==="; grep -A1 'fn name()' peri-middlewares/src/tools/filesystem/$f 2>/dev/null || grep -A1 'fn name()' peri-middlewares/src/tools/$f 2>/dev/null; done; echo "=== terminal.rs ==="; grep -A1 'fn name()' peri-middlewares/src/middleware/terminal.rs; echo "=== subagent/tool.rs ==="; grep -A1 'fn name()' peri-middlewares/src/subagent/tool.rs`
...
- `grep -n 'fn name()' peri-middlewares/src/tools/filesystem/folder.rs`
...
- `grep -n 'pub struct GrepTool\|pub struct SearchFilesRgTool' peri-middlewares/src/tools/filesystem/grep.rs`
...
- `cargo test -p peri-middlewares --lib -- "tools::filesystem::grep::tests::test_grep_hit" 2>&1 | tail -3`
- → 期望包含: `ok`
- **来源:** plan-2 Task 6 / design 验收标准
- **目的:** 确认参数结构重写生效

#### - [x] 4.3 Grep type 字段过滤正确
- `cargo test -p peri-middlewares --lib -- "test_grep_type_filter" 2>&1 | tail -3`
- → 期望包含: `ok`
...
- `cargo test -p peri-middlewares --lib -- "test_bash_timeout_returns_quickly" 2>&1 | tail -3`
- → 期望包含: `ok`
- **来源:** plan-2 Task 5
- **目的:** 确认 timeout 单位从秒迁移到毫秒

#### - [x] 5.2 Bash description 和 run_in_background 参数可解析
- `cargo test -p peri-middlewares --lib -- "test_bash_description_and_run_in_background_parsed" 2>&1 | tail -3`
- → 期望包含: `ok`
...
- `cargo test -p peri-middlewares --lib -- "subagent::tool::tests::test_tool_name" 2>&1 | tail -3`
- → 期望包含: `ok`
- **来源:** plan-2 Task 7
- **目的:** 确认工具名对齐

#### - [x] 6.2 Agent required 参数仅包含 prompt
- `cargo test -p peri-middlewares --lib -- "test_agent_parameters_required_is_prompt_only" 2>&1 | tail -3`
- → 期望包含: `ok`
- **来源:** plan-2 Task 7 新增测试
- **目的:** 确认参数 schema 重写

#### - [x] 6.3 filter_tools 防递归排除 Agent 而非 launch_agent
- `cargo test -p peri-middlewares --lib -- "test_agent_excluded_even_when_explicitly_allowed" 2>&1 | tail -3`
- → 期望包含: `ok`
- **来源:** plan-2 Task 7
- **目的:** 确认防递归逻辑正确

#### - [x] 6.4 Agent 预留字段（isolation/run_in_background）可解析
- `cargo test -p peri-middlewares --lib -- "test_agent_reserved_fields_parsed" 2>&1 | tail -3`
- → 期望包含: `ok`
- **来源:** plan-2 Task 7 新增测试
- **目的:** 确认预留字段不影响执行

#### - [x] 6.5 mod.rs 中 build_agents_summary 引用新工具名和参数
- `grep -n 'Agent\|subagent_type\|prompt' peri-middlewares/src/subagent/mod.rs | head -5`
...
- `cargo test -p peri-middlewares --lib -- "test_pdf_with_pages_returns_placeholder" 2>&1 | tail -3`
- → 期望包含: `ok`
- **来源:** plan-1 Task 2
- **目的:** 确认 PDF 占位逻辑生效

#### - [x] 7.2 PDF 无 pages 走二进制检测
- `cargo test -p peri-middlewares --lib -- "test_pdf_without_pages_returns_binary" 2>&1 | tail -3`
- → 期望包含: `ok`
- **来源:** plan-1 Task 2
- **目的:** 确认原有二进制检测不受影响
...
- `cargo test -p peri-middlewares --lib -- "test_multi_select_camel_case_input" 2>&1 | tail -3`
- → 期望包含: `ok`
- **来源:** plan-1 Task 3
- **目的:** 确认 camelCase 字段名映射

#### - [x] 8.2 preview 字段可解析不影响执行
- `cargo test -p peri-middlewares --lib -- "test_preview_field_ignored" 2>&1 | tail -3`
- → 期望包含: `ok`
- **来源:** plan-1 Task 3
...
- `grep -n 'HashMap\|\.id\b\|old_map\|new_map' peri-middlewares/src/tools/todo.rs`
- → 期望精确: （无输出）
- **来源:** plan-1 Task 4
...
- `grep -A15 'pub fn default_requires_approval' peri-middlewares/src/hitl/mod.rs`
...
- `grep -A5 'pub fn is_edit_tool' peri-middlewares/src/hitl/mod.rs`
- → 期望包含: `== "Write"`, `== "Edit"`
...
- `cargo test -p peri-tui --lib -- "test_format_tool_name" 2>&1 | tail -5`
- → 期望包含: `ok`（如果测试存在）
- **来源:** plan-2 Task 8
- **目的:** 确认工具显示名映射
...
- `cargo test -p peri-tui --lib -- "test_tool_category" 2>&1 | tail -5`
- → 期望包含: `ok`（如果测试存在）
- **来源:** plan-2 Task 8
- **目的:** 确认工具分类使用新名称
...
- `cargo test -p peri-middlewares --lib -- "test_grep_missing_pattern" 2>&1 | tail -3`
- → 期望包含: `ok`
- **来源:** plan-2 Task 6
- **目的:** 确认旧 args 接口已废弃

#### - [x] 12.3 Agent 缺少 prompt 时返回错误
- `cargo test -p peri-middlewares --lib -- "test_agent_prompt_missing_returns_error" 2>&1 | tail -3`
- → 期望包含: `ok`
- **来源:** plan-2 Task 7 新增测试
- **目的:** 确认必填参数校验
