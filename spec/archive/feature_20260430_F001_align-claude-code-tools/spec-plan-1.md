...
本次计划覆盖 Task 1-4，涉及 `peri-middlewares/src/tools/filesystem/`（Write/Edit/Glob/Read 四个工具定义）、`peri-middlewares/src/tools/`（AskUserTool、TodoWrite）、`peri-tui/src/app/agent.rs`（AskUserQuestion 事件路由）共 7 个源文件的工具名和参数结构对齐。按工具粒度拆分 Task 以保证原子性和可回滚性。
...
**背景:**
确保构建和测试工具链在当前开发环境中可用，避免后续 Task 因环境问题阻塞。本文件涵盖 `peri-middlewares` 和 `peri-tui` 两个 crate，需验证全量构建和中间件测试。
...
- [ ] 验证中间件 crate 测试框架可用（dry run）
  - `cargo test -p peri-middlewares --no-run`
  - 预期: 测试编译成功，无配置错误
...
- [ ] 修改 WriteFileTool 的工具名 — 将 `fn name()` 返回值从 `"write_file"` 改为 `"Write"`
  - 位置: `peri-middlewares/src/tools/filesystem/write.rs` → `impl BaseTool for WriteFileTool` 的 `fn name()` (~L33)
...
- [ ] 更新 WriteFileTool 描述中引用 `read_file` 的文本 — 描述中提到 "use the read_file tool"，`read_file` 将在 Task 2 中改名为 `Read`，此处同步更新以保持一致
  - 位置: `peri-middlewares/src/tools/filesystem/write.rs` → `WRITE_FILE_DESCRIPTION` 常量 (~L10)
  - 将 `read_file tool` 改为 `Read tool`
  - 原因: 描述文本需引用新工具名

- [ ] 修改 EditFileTool 的工具名 — 将 `fn name()` 返回值从 `"edit_file"` 改为 `"Edit"`
  - 位置: `peri-middlewares/src/tools/filesystem/edit.rs` → `impl BaseTool for EditFileTool` 的 `fn name()` (~L37)
  - 将 `"edit_file"` 改为 `"Edit"`
  - 原因: 对齐 Claude Code 工具命名

- [ ] 更新 EditFileTool 描述中引用 `read_file` 的文本
  - 位置: `peri-middlewares/src/tools/filesystem/edit.rs` → `EDIT_FILE_DESCRIPTION` 常量 (~L9, L10)
  - 将 `read_file tool` 改为 `Read tool`（共 2 处）
  - 原因: 描述文本需引用新工具名

- [ ] 修改 GlobFilesTool 的工具名 — 将 `fn name()` 返回值从 `"glob_files"` 改为 `"Glob"`
  - 位置: `peri-middlewares/src/tools/filesystem/glob.rs` → `impl BaseTool for GlobFilesTool` 的 `fn name()` (~L99)
  - 将 `"glob_files"` 改为 `"Glob"`
  - 原因: 对齐 Claude Code 工具命名

- [ ] 更新 GlobFilesTool 描述中引用旧工具名的文本
  - 位置: `peri-middlewares/src/tools/filesystem/glob.rs` → `GLOB_FILES_DESCRIPTION` 常量 (~L29, L33, L34)
...
- [ ] 为三个工具的新名称编写单元测试
  - 测试文件: `peri-middlewares/src/tools/filesystem/write.rs`、`edit.rs`、`glob.rs` 各自的 `#[cfg(test)] mod tests` 块
  ...
  - 运行命令: `cargo test -p peri-middlewares --lib -- tools::filesystem::write::tests::test_tool_name_is_Write tools::filesystem::edit::tests::test_tool_name_is_Edit tools::filesystem::glob::tests::test_tool_name_is_Glob`
  - 预期: 三个测试全部通过

**检查步骤:**

- [ ] 验证三个工具的 `fn name()` 返回新名称
  - `cargo test -p peri-middlewares --lib -- test_tool_name_is`
  - 预期: 三个测试通过（Write / Edit / Glob）
- [ ] 验证描述文本中无旧工具名残留
  - `grep -n 'read_file\|edit_file\|glob_files\|search_files_rg\|launch_agent' peri-middlewares/src/tools/filesystem/write.rs peri-middlewares/src/tools/filesystem/edit.rs peri-middlewares/src/tools/filesystem/glob.rs`
...
- 修改: `peri-middlewares/src/middleware/filesystem.rs`（更新 `tool_names()` 中 `"read_file"` → `"Read"`）

**执行步骤:**

- [ ] 修改 `fn name()` 返回值从 `"read_file"` 改为 `"Read"` — 对齐 Claude Code 工具命名
  - 位置: `peri-middlewares/src/tools/filesystem/read.rs` → `impl BaseTool for ReadFileTool` 的 `fn name()` (~L83)
  - 将 `"read_file"` 改为 `"Read"`

- [ ] 更新 `READ_FILE_DESCRIPTION` 常量中的旧工具名引用 — 描述文本中引用的工具名需全部对齐新名称
  - 位置: `peri-middlewares/src/tools/filesystem/read.rs` → `READ_FILE_DESCRIPTION` 常量 (~L21-39)
...
- [ ] 在 `fn parameters()` 的 JSON schema 中新增 `pages` 参数 — 支持 PDF 页范围读取
  - 位置: `peri-middlewares/src/tools/filesystem/read.rs` → `fn parameters()` (~L91-110)
  - 在 `properties` 对象中追加:
...
- [ ] 在 `fn invoke()` 中增加 PDF + pages 判断逻辑 — 在二进制检测之前拦截 PDF + pages 场景
  - 位置: `peri-middlewares/src/tools/filesystem/read.rs` → `fn invoke()` (~L112-174)
...
- [ ] 为 Read 工具新名称和 PDF 逻辑编写单元测试
  - 测试文件: `peri-middlewares/src/tools/filesystem/read.rs` → `#[cfg(test)] mod tests` 块
  - 在现有测试末尾追加以下测试函数：
  ...
  - 运行命令: `cargo test -p peri-middlewares --lib -- tools::filesystem::read::tests::test_tool_name_is_Read tools::filesystem::read::tests::test_pdf_with_pages_returns_placeholder tools::filesystem::read::tests::test_pdf_without_pages_returns_binary`
  - 预期: 三个测试全部通过

**检查步骤:**

- [ ] 验证 `fn name()` 返回 `"Read"`
  - `cargo test -p peri-middlewares --lib -- tools::filesystem::read::tests::test_tool_name_is_Read`
  - 预期: 测试通过
- [ ] 验证描述文本中无旧工具名残留
  - `grep -n 'read_file\|search_files_rg\|launch_agent' peri-middlewares/src/tools/filesystem/read.rs`
  - 预期: 无匹配输出（所有旧工具名已从描述中清除）
- [ ] 验证 PDF 逻辑测试通过
  - `cargo test -p peri-middlewares --lib -- test_pdf_with_pages_returns_placeholder test_pdf_without_pages_returns_binary`
  - 预期: 两个测试通过
- [ ] 验证模块编译和全量测试通过
  - `cargo test -p peri-middlewares`
  - 预期: 所有测试通过，无编译错误
...
**背景:**
将 `ask_user_question` 工具重命名为 `AskUserQuestion`，并对齐字段命名：`multi_select` → `multiSelect`（camelCase），在选项中新增 `preview` 字段。工具定义来自 `ask_user_tool_definition()` 函数（位于 `peri-middlewares/src/ask_user/mod.rs`），工具实现位于 `peri-middlewares/src/tools/ask_user_tool.rs`。两个文件中有重复的 `InputOption`/`InputQuestion` 反序列化结构体，需同步修改 serde rename。`peri-tui/src/app/agent.rs` 中 `map_executor_event` 函数通过工具名字符串匹配 `ask_user_question`，需同步更新为 `"AskUserQuestion"`。本 Task 不依赖其他 Task；Task 5（验收）依赖本 Task 完成。
...
- [ ] 修改 `ask_user_tool_definition()` 中的工具名 — 从 `"ask_user_question"` 改为 `"AskUserQuestion"`
  - 位置: `peri-middlewares/src/ask_user/mod.rs` → `ask_user_tool_definition()` 函数 (~L63-123)
...
- [ ] 修改 `ask_user_tool_definition()` 参数 schema 中的字段名 — `multi_select` → `multiSelect`
  - 位置: `peri-middlewares/src/ask_user/mod.rs` → `ask_user_tool_definition()` 的 `parameters` JSON (~L71-121)
...
- [ ] 修改 `parse_ask_user()` 中的工具名匹配 — 从 `"ask_user_question"` 改为 `"AskUserQuestion"`
  - 位置: `peri-middlewares/src/ask_user/mod.rs` → `parse_ask_user()` 函数 (~L31)
...
- [ ] 修改 `InputQuestion` 反序列化结构体的 serde rename — 使 LLM 传入 `multiSelect` 时能正确反序列化为 `multi_select` 字段
  - 位置: `peri-middlewares/src/ask_user/mod.rs` → `InputQuestion` 结构体 (~L15-22)
...
- [ ] 修改 `InputOption` 反序列化结构体 — 新增 `preview` 字段
  - 位置: `peri-middlewares/src/ask_user/mod.rs` → `InputOption` 结构体 (~L9-12)
  - 在 `description` 字段后追加 `preview: Option<String>` 字段

- [ ] 修改 `parse_ask_user()` 中的选项映射 — 传递 `preview` 字段
  - 位置: `peri-middlewares/src/ask_user/mod.rs` → `parse_ask_user()` 函数内 `.map(|o| AskUserOption { ... })` (~L51-54)
  - `AskUserOption` 结构体当前只有 `label` 和 `description` 两个字段，`preview` 暂不传递（`AskUserOption` 是核心框架 `peri-agent` 的类型，不在本 Task 范围内修改），此处忽略 `o.preview` 即可

- [ ] 修改 `ask_user_tool.rs` 中的 `fn name()` — 从 `"ask_user_question"` 改为 `"AskUserQuestion"`
  - 位置: `peri-middlewares/src/tools/ask_user_tool.rs` → `impl BaseTool for AskUserTool` 的 `fn name()` (~L78)
  - 将 `"ask_user_question"` 改为 `"AskUserQuestion"`

- [ ] 修改 `ask_user_tool.rs` 中 `InputQuestion` 反序列化结构体的 serde rename — 与 `mod.rs` 保持一致
  - 位置: `peri-middlewares/src/tools/ask_user_tool.rs` → `InputQuestion` 结构体 (~L36-43)
  - 在 `multi_select` 字段上添加 `#[serde(default, rename = "multiSelect")]`

- [ ] 修改 `ask_user_tool.rs` 中 `InputOption` 反序列化结构体 — 新增 `preview` 字段
  - 位置: `peri-middlewares/src/tools/ask_user_tool.rs` → `InputOption` 结构体 (~L30-34)
  - 在 `description` 字段后追加 `preview: Option<String>` 字段

- [ ] 修改 `agent.rs` 中的工具名匹配 — 从 `"ask_user_question"` 改为 `"AskUserQuestion"`
  - 位置: `peri-tui/src/app/agent.rs` → `map_executor_event()` 函数
...
- [ ] 为 AskUserQuestion 工具新名称和字段对齐编写单元测试
  - 测试文件: `peri-middlewares/src/tools/ask_user_tool.rs` → `#[cfg(test)] mod tests` 块
  - 在现有测试末尾追加以下测试函数：
  ...
  - 运行命令: `cargo test -p peri-middlewares --lib -- tools::ask_user_tool::tests::test_tool_name_is_AskUserQuestion tools::ask_user_tool::tests::test_multi_select_camel_case_input tools::ask_user_tool::tests::test_preview_field_ignored`
  - 预期: 三个测试全部通过

**检查步骤:**

- [ ] 验证工具名返回 `"AskUserQuestion"`
  - `cargo test -p peri-middlewares --lib -- test_tool_name_is_AskUserQuestion`
  - 预期: 测试通过
- [ ] 验证 camelCase 字段名和 preview 字段正常工作
  - `cargo test -p peri-middlewares --lib -- test_multi_select_camel_case_input test_preview_field_ignored`
  - 预期: 两个测试通过
- [ ] 验证源码中无残留的旧工具名（排除注释和错误信息）
  - `grep -n '"ask_user_question"' peri-middlewares/src/ask_user/mod.rs peri-middlewares/src/tools/ask_user_tool.rs`
  - 预期: 无匹配输出
- [ ] 验证 agent.rs 中工具名匹配已更新
  - `grep -n 'ask_user_question' peri-tui/src/app/agent.rs`
  - 预期: 仅在注释中出现（如有），无字符串字面量匹配
- [ ] 验证模块编译和全量测试通过
  - `cargo test -p peri-middlewares -p peri-tui`
  - 预期: 所有测试通过，无编译错误
...
**背景:**
将 `todo_write` 工具重命名为 `TodoWrite`，移除 `TodoItem` 的 `id` 字段（全量替换语义下用数组索引标识），新增 `activeForm: Option<String>` 字段（进行时形式，用于 UI spinner 展示）。`summarize_changes()` 函数当前基于 `id` 的 HashMap 对比新旧列表，需改为基于数组索引对比。`TodoItem` 被 TUI 层使用（`peri-tui/src/app/mod.rs` 的 `todo_items` 字段、`peri-tui/src/ui/main_ui.rs` 的渲染逻辑），经代码确认 TUI 层仅访问 `content` 和 `status` 字段，不访问 `id`，因此移除 `id` 不影响 TUI 渲染。本 Task 不依赖其他 Task；Task 5（验收）依赖本 Task 完成。
...
- [ ] 修改 `fn name()` 返回值从 `"todo_write"` 改为 `"TodoWrite"` — 对齐 Claude Code 工具命名
  - 位置: `peri-middlewares/src/tools/todo.rs` → `impl BaseTool for TodoWriteTool` 的 `fn name()` (~L125)
  - 将 `"todo_write"` 改为 `"TodoWrite"`

- [ ] 修改 `TodoItem` 结构体 — 移除 `id` 字段，新增 `activeForm` 字段
  - 位置: `peri-middlewares/src/tools/todo.rs` → `TodoItem` 结构体 (~L20-25)
  - 移除 `pub id: String` 字段
...
- [ ] 修改 `fn parameters()` 的 JSON schema — 移除 `id`，新增 `activeForm`
  - 位置: `peri-middlewares/src/tools/todo.rs` → `fn parameters()` (~L133-163)
...
- [ ] 重写 `summarize_changes()` 函数 — 改为基于数组索引对比而非 id HashMap
  - 位置: `peri-middlewares/src/tools/todo.rs` → `fn summarize_changes()` (~L70-121)
  - 将整个函数体替换为基于索引的对比逻辑：
...
- [ ] 为 TodoWrite 工具新名称、无 id 结构和索引对比逻辑编写单元测试
  ...
  - 运行命令: `cargo test -p peri-middlewares --lib -- tools::todo::tests`
  - 预期: 所有测试通过

**检查步骤:**

- [ ] 验证工具名返回 `"TodoWrite"`
  - `cargo test -p peri-middlewares --lib -- tools::todo::tests::test_tool_name_is_TodoWrite`
  - 预期: 测试通过
- [ ] 验证 TodoItem 不再包含 id 字段
  - `grep -n 'pub id:' peri-middlewares/src/tools/todo.rs`
  - 预期: 无匹配输出
- [ ] 验证 summarize_changes 使用索引对比
  - `grep -n 'HashMap\|\.id\b\|id\.as_str\|old_map\|new_map' peri-middlewares/src/tools/todo.rs`
  - 预期: 无匹配输出（所有基于 id 的 HashMap 逻辑已移除）
- [ ] 验证 activeForm 字段存在
  - `grep -n 'active_form\|activeForm' peri-middlewares/src/tools/todo.rs`
...
2. 验证旧工具名在中间件源码描述文本中无残留
   - `grep -rn '"write_file"\|"edit_file"\|"glob_files"\|"read_file"\|"ask_user_question"\|"todo_write"' peri-middlewares/src/tools/ --include='*.rs'`
...
3. 验证新工具名在工具定义中正确生效
   - `grep -rn '"Write"\|"Edit"\|"Glob"\|"Read"\|"AskUserQuestion"\|"TodoWrite"' peri-middlewares/src/tools/ --include='*.rs'`
...
5. 验证 TodoWrite 不再包含 id 字段且 summarize_changes 使用索引对比
   - `grep -n 'pub id:\|HashMap\|old_map\|new_map' peri-middlewares/src/tools/todo.rs`
   - 预期: 无匹配输出
