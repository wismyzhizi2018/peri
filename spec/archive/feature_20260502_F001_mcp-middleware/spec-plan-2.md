...
- [ ] 验证 Plan 1 核心组件编译通过
  ...
  - 预期: `Finished` 无错误
- [ ] 验证 mcp 模块已有 5 个子文件
  - `ls rust-agent-middlewares/src/mcp/`
...
**涉及文件:**
- 新建: `rust-agent-middlewares/src/mcp/resource_tool.rs`
- 修改: `rust-agent-middlewares/src/mcp/mod.rs`（添加 `pub mod resource_tool` + 重导出）

**执行步骤:**

- [ ] 在 `resource_tool.rs` 顶部定义 `McpResourceTool` 结构体和 `ResourceError` 错误类型
  - 位置: 新建 `rust-agent-middlewares/src/mcp/resource_tool.rs`，文件顶部
  - 关键逻辑:
    ```rust
    use std::sync::Arc;
...
- [ ] 为 McpResourceTool 资源读取工具编写单元测试
  - 测试文件: `rust-agent-middlewares/src/mcp/resource_tool.rs`（文件底部 `#[cfg(test)] mod tests` 块）
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib -- mcp::resource_tool::tests`
  - 预期: 所有测试通过
...
- [ ] 验证 resource_tool.rs 单元测试全部通过
  - `cargo test -p rust-agent-middlewares --lib -- mcp::resource_tool::tests 2>&1 | tail -15`
  - 预期: 所有 `test_*` 测试通过，输出 `test result: ok`

- [ ] 验证 mod.rs 正确声明 resource_tool 子模块
  - `grep -n "pub mod resource_tool" rust-agent-middlewares/src/mcp/mod.rs`
  - 预期: 输出包含 `pub mod resource_tool;`

- [ ] 验证 mod.rs 正确重导出 resource_tool 类型
  - `grep -n "pub use resource_tool" rust-agent-middlewares/src/mcp/mod.rs`
  - 预期: 输出包含 `pub use resource_tool::{McpResourceTool, ResourceError};`

- [ ] 验证 McpResourceTool 实现了 BaseTool trait
  - `grep -n "impl BaseTool for McpResourceTool" rust-agent-middlewares/src/mcp/resource_tool.rs`
  - 预期: 输出包含 `impl BaseTool for McpResourceTool`

- [ ] 验证 invoke 方法包含 120s 超时
  - `grep -n "RESOURCE_READ_TIMEOUT\|from_secs(120)" rust-agent-middlewares/src/mcp/resource_tool.rs`
...
**涉及文件:**
- 新建: `rust-agent-middlewares/src/mcp/middleware.rs`
- 修改: `rust-agent-middlewares/src/mcp/mod.rs`（添加 `pub mod middleware` + 重导出 McpMiddleware）
- 修改: `rust-agent-middlewares/src/lib.rs`（添加 `pub use mcp::McpMiddleware`）

**执行步骤:**

- [ ] 新建 `middleware.rs` 并定义 `McpMiddleware` 结构体
  - 位置: 新建 `rust-agent-middlewares/src/mcp/middleware.rs`，文件顶部
  - 关键逻辑:
    ```rust
    use std::sync::Arc;
    use async_trait::async_trait;
...
- [ ] 修改 `mcp/mod.rs` 添加 `pub mod middleware` 声明和重导出
  - 位置: `rust-agent-middlewares/src/mcp/mod.rs`，在最后一个 `pub mod` 声明之后追加
  - 追加内容:
    ```rust
    pub mod middleware;
...
- [ ] 修改 `lib.rs` 添加 McpMiddleware 重导出
  - 位置: `rust-agent-middlewares/src/lib.rs`，在现有的 `pub use mcp::{...}` 行中追加 `McpMiddleware`
  - 修改方式: 找到 `pub use mcp::{` 开头的行，在闭合 `};` 前追加 `McpMiddleware,`
  - 原因: 与现有模块重导出模式一致，使外部 crate 可通过 `rust_agent_middlewares::McpMiddleware` 直接引用

- [ ] 为 McpMiddleware 中间件编写单元测试
  - 测试文件: `rust-agent-middlewares/src/mcp/middleware.rs`（文件底部 `#[cfg(test)] mod tests` 块）
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib -- mcp::middleware::tests`
  - 预期: 所有测试通过
...
- [ ] 验证 middleware.rs 单元测试全部通过
  - `cargo test -p rust-agent-middlewares --lib -- mcp::middleware::tests 2>&1 | tail -15`
  - 预期: 所有 `test_*` 测试通过，输出 `test result: ok`

- [ ] 验证 mod.rs 正确声明 middleware 子模块
  - `grep -n "pub mod middleware" rust-agent-middlewares/src/mcp/mod.rs`
  - 预期: 输出包含 `pub mod middleware;`

- [ ] 验证 mod.rs 正确重导出 McpMiddleware
  - `grep -n "pub use middleware" rust-agent-middlewares/src/mcp/mod.rs`
  - 预期: 输出包含 `pub use middleware::McpMiddleware;`

- [ ] 验证 lib.rs 正确重导出 McpMiddleware
  - `grep -n "McpMiddleware" rust-agent-middlewares/src/lib.rs`
  - 预期: 输出包含 `McpMiddleware`

- [ ] 验证 McpMiddleware 实现了 Middleware trait
  - `grep -n "impl.*Middleware.*for McpMiddleware" rust-agent-middlewares/src/mcp/middleware.rs`
  - 预期: 输出包含 `impl<S: State> Middleware<S> for McpMiddleware`

- [ ] 验证 collect_tools 调用了 build_tool_bridges
  - `grep -n "build_tool_bridges" rust-agent-middlewares/src/mcp/middleware.rs`
  - 预期: 输出包含 `build_tool_bridges`，确认 collect_tools 使用了 Task 4 的工厂函数

- [ ] 验证 collect_tools 包含 McpResourceTool 条件判断
  - `grep -n "has_resources\|McpResourceTool" rust-agent-middlewares/src/mcp/middleware.rs`
  - 预期: 输出包含 `has_resources` 和 `McpResourceTool`，确认资源工具按条件注入

- [ ] 验证 before_agent / after_agent 未被覆盖（使用默认空实现）
  - `grep -n "before_agent\|after_agent" rust-agent-middlewares/src/mcp/middleware.rs`
...
[修改原因] — 当前 `default_requires_approval()` 函数（`rust-agent-middlewares/src/hitl/mod.rs:40-48`）仅匹配内置工具名，不含任何 MCP 工具名匹配规则。MCP 工具以 `mcp__` 为前缀命名（格式 `mcp__{server}__{tool}`），需增加前缀通配匹配使其走标准 HITL 审批流程。`mcp_read_resource` 是只读操作（类比内置 `Read` 工具不拦截），不以 `mcp__` 开头（双下划线），不在拦截范围内。
...
- [ ] 在 `default_requires_approval()` 函数中添加 `mcp__` 前缀匹配规则
  - 位置: `rust-agent-middlewares/src/hitl/mod.rs:40-48`，`default_requires_approval()` 函数体
...
- [ ] 在 `test_default_requires_approval` 测试中追加 MCP 工具名断言
  - 位置: `rust-agent-middlewares/src/hitl/mod.rs:454-469`，`test_default_requires_approval()` 函数体
...
- [ ] 为 default_requires_approval 的 MCP 匹配逻辑编写单元测试
  - 测试文件: `rust-agent-middlewares/src/hitl/mod.rs`（文件底部 `#[cfg(test)] mod tests` 块，追加新测试函数）
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-agent-middlewares --lib -- hitl::tests::test_mcp`
  - 预期: 所有测试通过

**检查步骤:**

- [ ] 验证 default_requires_approval 包含 mcp__ 前缀匹配
  - `grep -n 'starts_with("mcp__")' rust-agent-middlewares/src/hitl/mod.rs`
  - 预期: 输出包含 `starts_with("mcp__")`，确认匹配规则已添加

- [ ] 验证 is_edit_tool 未被修改（MCP 工具不属于编辑工具）
  - `grep -n 'mcp' rust-agent-middlewares/src/hitl/mod.rs`
...
- [ ] 验证 hitl 模块全部测试通过（含新增 MCP 断言）
  - `cargo test -p rust-agent-middlewares --lib -- hitl::tests 2>&1 | tail -20`
  - 预期: 所有 `test_*` 测试通过，输出 `test result: ok`
...
**涉及文件:**
- 修改: `rust-agent-tui/src/app/mod.rs`（App 结构体新增 `mcp_pool` 字段 + 退出清理）
- 修改: `rust-agent-tui/src/app/agent.rs`（AgentRunConfig 新增 `mcp_pool` 字段 + 中间件注册 + parent_tools 扩展）
- 修改: `rust-agent-tui/src/app/agent_ops.rs`（惰性初始化 pool 并传入 AgentRunConfig）
- 修改: `rust-agent-tui/src/app/panel_ops.rs`（`new_headless()` 补充 `mcp_pool: None` 字段）
- 修改: `rust-agent-tui/src/main.rs`（App 退出时调用 `mcp_pool.shutdown()`）

**执行步骤:**

- [ ] 在 App 结构体中新增 `mcp_pool` 字段
  - 位置: `rust-agent-tui/src/app/mod.rs:73-93`，`App` 结构体定义中，在 `config_path_override` 字段之后追加
  - 关键逻辑:
    ```rust
...
- [ ] 在 `App::new()` 构造函数中将 `mcp_pool` 初始化为 None
  - 位置: `rust-agent-tui/src/app/mod.rs:154-183`，`Self { ... }` 构造块中，在 `config_path_override: None,` 行之后追加
  - 追加内容:
    ```rust
    mcp_pool: None,
    ```
...
- [ ] 在 `AgentRunConfig` 结构体中新增 `mcp_pool` 字段
  - 位置: `rust-agent-tui/src/app/agent.rs:18-34`，`AgentRunConfig` 定义中，在 `permission_mode` 字段之后追加
  - 关键逻辑:
    ```rust
    pub mcp_pool: Option<Arc<rust_agent_middlewares::mcp::McpClientPool>>,
    ```
...
- [ ] 在 `run_universal_agent()` 函数解构中提取 `mcp_pool`
  - 位置: `rust-agent-tui/src/app/agent.rs:36-52`，`let AgentRunConfig { ... } = cfg;` 解构块中，在 `permission_mode,` 行之后追加
  - 追加内容:
    ```rust
    mcp_pool,
    ```
...
- [ ] 在 `run_universal_agent()` 中构建 parent_tools 时包含 MCP 工具
  - 位置: `rust-agent-tui/src/app/agent.rs:164-166`，`parent_tools` 构建处，在 `parent_tools.extend(TerminalMiddleware::build_tools(&cwd));` 之后追加
  - 关键逻辑:
    ```rust
    // 将 MCP 工具加入 parent_tools，供 SubAgent 继承
    if let Some(ref pool) = mcp_pool {
        let mcp_tools = rust_agent_middlewares::mcp::McpMiddleware::new(Arc::clone(pool))
            .collect_tools(&cwd);
...
- [ ] 在 `run_universal_agent()` 中注册 McpMiddleware 到中间件链
  - 位置: `rust-agent-tui/src/app/agent.rs:232-233`，在 `.add_middleware(Box::new(subagent))` 之后、`.with_event_handler(Arc::clone(&handler))` 之前追加
  - 关键逻辑:
    ```rust
...
- [ ] 在 `submit_message()` 中惰性初始化 MCP pool 并传入 AgentRunConfig
  - 位置: `rust-agent-tui/src/app/agent_ops.rs:141-162`，`tokio::spawn` 闭包中构建 `AgentRunConfig` 的位置
    ...
    if self.mcp_pool.is_none() {
        ...
        let pool = tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(
                rust_agent_middlewares::mcp::McpClientPool::initialize(&pool_cwd)
            )
        });
        match pool {
            Ok(p) => {
...
- [ ] 在 `new_headless()` 中补充 `mcp_pool: None` 字段
  - 位置: `rust-agent-tui/src/app/panel_ops.rs:300-321`，`App { ... }` 构造块中，在 `config_path_override: Some(test_config_path),` 行之后追加
  - 追加内容:
    ```rust
    mcp_pool: None,
    ```
...
- [ ] 在 App 退出时调用 `mcp_pool.shutdown()` 清理所有 MCP 连接
  - 位置: `rust-agent-tui/src/main.rs:226-233`，`'event_loop` 退出后、Langfuse flush 之前
  - 关键逻辑:
    ```rust
...
- [ ] 为 TUI MCP 集成编写单元测试
  - 测试文件: `rust-agent-tui/src/app/agent_ops.rs`（文件底部 `#[cfg(test)] mod tests` 块追加新测试函数）
  - 测试场景:
  ...
  - 运行命令: `cargo test -p rust-agent-tui --lib -- app::agent_ops::tests::test_mcp 2>&1 | tail -15`
  - 预期: 所有测试通过

**检查步骤:**

- [ ] 验证 App 结构体包含 mcp_pool 字段
  - `grep -n "mcp_pool" rust-agent-tui/src/app/mod.rs`
  - 预期: 输出包含 `pub mcp_pool: Option<Arc<rust_agent_middlewares::mcp::McpClientPool>>,`

- [ ] 验证 App::new() 初始化 mcp_pool 为 None
  - `grep -n "mcp_pool: None" rust-agent-tui/src/app/mod.rs`
  - 预期: 输出包含 `mcp_pool: None,`

- [ ] 验证 AgentRunConfig 包含 mcp_pool 字段
  - `grep -n "mcp_pool" rust-agent-tui/src/app/agent.rs`
  - 预期: 输出至少 3 行：结构体字段定义、解构提取、条件注册

- [ ] 验证 McpMiddleware 在 SubAgentMiddleware 之后注册
  - `grep -n -A2 "add_middleware.*subagent" rust-agent-tui/src/app/agent.rs`
  - 预期: subagent 注册之后紧跟 McpMiddleware 的条件注册代码

- [ ] 验证 parent_tools 包含 MCP 工具扩展逻辑
  - `grep -n "collect_tools" rust-agent-tui/src/app/agent.rs`
  - 预期: 输出包含 parent_tools 构建中的 MCP collect_tools 调用

- [ ] 验证 submit_message 包含 pool 惰性初始化逻辑
  - `grep -n "McpClientPool::initialize\|mcp_pool" rust-agent-tui/src/app/agent_ops.rs`
  - 预期: 输出包含 `McpClientPool::initialize` 调用和 `mcp_pool` 传递

- [ ] 验证 new_headless 补充了 mcp_pool 字段
  - `grep -n "mcp_pool" rust-agent-tui/src/app/panel_ops.rs`
  - 预期: 输出包含 `mcp_pool: None,`

- [ ] 验证 main.rs 包含 MCP pool shutdown 逻辑
  - `grep -n "mcp_pool\|shutdown" rust-agent-tui/src/main.rs`
...
- [ ] 验证新增测试通过
  - `cargo test -p rust-agent-tui --lib -- app::agent_ops::tests::test_mcp 2>&1 | tail -15`
