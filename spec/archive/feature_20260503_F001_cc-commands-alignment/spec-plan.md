...
**技术栈:** Rust 2021, ratatui, peri-widgets (TabBar/BorderedPanel/ScrollableArea), tokio, serde
...
- [x] 验证测试工具可用
  - `cargo test -p peri-tui --lib -- command::tests 2>&1 | tail -3`
...
- 修改: `peri-tui/src/command/clear.rs`

**执行步骤:**

- [x] 在 Command trait 中新增 `aliases()` 默认方法
  - 位置: `peri-tui/src/command/mod.rs` trait 定义块，`description()` 之后、`execute()` 之前（~L36-37 之间）
  - 内容:
...
- [x] 在 `dispatch` 方法中插入别名精确匹配（优先级第 2 级）
  - 位置: `peri-tui/src/command/mod.rs` `dispatch()` 方法，精确匹配 `name` 块（~L68-72）之后、前缀唯一匹配块（~L75-83）之前
...
- [x] 扩展 `dispatch` 前缀匹配以覆盖别名
  - 位置: `peri-tui/src/command/mod.rs` `dispatch()` 方法，前缀匹配块（~L75-83）
...
- [x] 扩展 `list()` 方法返回别名信息
  - 位置: `peri-tui/src/command/mod.rs` `list()` 方法（~L89-94）
...
- [x] 同步更新 `core.rs` 中 `command_help_list` 的构建逻辑
  - 位置: `peri-tui/src/app/core.rs` ~L80-83
...
- [x] 同步更新 `help.rs` 显示逻辑以展示别名
  - 位置: `peri-tui/src/command/help.rs` ~L18-19
...
- [x] 扩展 `match_prefix()` 方法以覆盖别名
  - 位置: `peri-tui/src/command/mod.rs` `match_prefix()` 方法（~L98-104）
...
- [x] 为 ClearCommand 添加 aliases
  - 位置: `peri-tui/src/command/clear.rs` ~L6-18，在 `description()` 和 `execute()` 之间
  - 内容: 添加 `aliases()` 实现:
...
- [x] 为 Command trait 别名匹配逻辑编写单元测试
  - 测试文件: `peri-tui/src/command/mod.rs`（现有 `#[cfg(test)] mod tests` 块末尾，~L276 之前）
  - 测试场景:
  ...
  - 运行命令: `cargo test -p peri-tui --lib -- command::tests`
  - 预期: 所有测试通过（含原有 8 个 + 新增 8 个）

**检查步骤:**

- [x] 验证 Command trait 包含 aliases 默认方法
  - `grep -n 'fn aliases' peri-tui/src/command/mod.rs`
  - 预期: 输出包含 `fn aliases(&self) -> Vec<&str>` 行

- [x] 验证 ClearCommand 实现了 aliases
  - `grep -n 'aliases\|reset\|new' peri-tui/src/command/clear.rs`
  - 预期: 输出包含 `vec!["reset", "new"]`

- [x] 验证 dispatch 包含别名精确匹配逻辑
  - `grep -n '别名精确匹配' peri-tui/src/command/mod.rs`
  - 预期: 输出匹配到对应注释行

- [x] 验证 help 命令显示别名
  - `grep -n 'alias_str\|别名' peri-tui/src/command/help.rs`
  - 预期: 输出包含别名拼接逻辑
...
- [x] 全部测试通过
  - `cargo test -p peri-tui --lib -- command::tests 2>&1 | tail -10`
...
- 新建: `peri-tui/src/app/config_panel.rs`（核心）
- 新建: `peri-tui/src/ui/main_ui/panels/config.rs`（核心）
- 新建: `peri-tui/src/command/config.rs`（核心）
- 修改: `peri-tui/src/config/types.rs`（AppConfig 增加 4 个字段）
- 注: app/mod.rs/core.rs/panel_ops.rs/command/mod.rs 的集成修改由 Task 5 统一执行

**执行步骤:**

- [x] 在 AppConfig 中新增 4 个配置字段
  - 位置: `peri-tui/src/config/types.rs` AppConfig 结构体（~L96-120），在 `compact` 字段之后、`extra` 字段之前插入
  - 新增字段：
...
- [x] 创建 ConfigPanel 状态结构体
  - 新建文件: `peri-tui/src/app/config_panel.rs`
...
- [x] 创建 ConfigCommand
  - 新建文件: `peri-tui/src/command/config.rs`
...
- [x] 创建 ConfigPanel 渲染函数
  - 新建文件: `peri-tui/src/ui/main_ui/panels/config.rs`
...
- [x] 为 AppConfig 新增字段编写序列化/反序列化单元测试
  ...
  - 运行命令: `cargo test -p peri-tui --lib -- config::types::tests::test_app_config_new`
  - 预期: 所有测试通过

- [x] 为 ConfigPanel 状态逻辑编写单元测试
  - 测试文件: `peri-tui/src/app/config_panel.rs` 的 `#[cfg(test)] mod tests` 块
  - 测试场景:
  ...
  - 运行命令: `cargo test -p peri-tui --lib -- config_panel::tests`
  - 预期: 所有测试通过

**检查步骤:**

- [x] 验证 AppConfig 新增字段不影响现有序列化兼容性
  - `cargo test -p peri-tui --lib -- config::types::tests`
  - 预期: 所有现有测试 + 新测试通过，无回归

- [x] 验证 ConfigPanel 状态逻辑正确
  - `cargo test -p peri-tui --lib -- config_panel::tests`
  - 预期: 所有 8 个测试通过
...
- [x] 验证 ConfigCommand 已注册
  - `grep -n 'config::ConfigCommand' peri-tui/src/command/mod.rs`
  - 预期: 输出包含注册行

- [x] 验证 config_panel 模块和渲染模块存在
  - `test -f peri-tui/src/app/config_panel.rs && echo "OK"`
  - `test -f peri-tui/src/ui/main_ui/panels/config.rs && echo "OK"`
  - `test -f peri-tui/src/command/config.rs && echo "OK"`
  - 预期: 三个文件均存在

- [x] 验证 AppCore 包含 config_panel 字段
  - `grep -n 'config_panel' peri-tui/src/app/core.rs`
...
- 新建: `peri-tui/src/app/status_panel.rs`（核心）
- 新建: `peri-tui/src/ui/main_ui/panels/status.rs`（核心）
- 新建: `peri-tui/src/command/cost.rs`（核心）
- 新建: `peri-tui/src/command/context_cmd.rs`（核心）
- 修改: `peri-tui/src/app/agent_comm.rs`（增加 session_start_time 和 tool_call_count 字段）
- 修改: `peri-tui/src/app/agent_ops.rs`（session_start_time 赋值和 tool_call_count 递增）
- 注: app/mod.rs/command/mod.rs 的模块注册由 Task 5 统一执行

**执行步骤:**

- [x] 在 `AgentComm` 中新增会话计时和工具调用计数字段 — 为面板提供数据来源
  - 位置: `peri-tui/src/app/agent_comm.rs:21-55` (AgentComm 结构体)
  ...
  - 位置: `peri-tui/src/app/agent_comm.rs:57-79` (Default impl)
...
- [x] 在 `submit_message` 中记录会话开始时间 — 首次提交时启动计时
  - 位置: `peri-tui/src/app/agent_ops.rs`，找到 `submit_message` 方法
...
- [x] 在 `handle_agent_event` 的 ToolStart 分支递增 `tool_call_count` — 每次工具调用计数
  - 位置: `peri-tui/src/app/agent_ops.rs:376-405` (ToolStart match 分支)
...
- [x] 创建 `StatusPanel` 状态结构 — 管理面板的 Tab 状态和滚动偏移
  - 新建: `peri-tui/src/app/status_panel.rs`
  - 内容:
...
- [x] 在 App 结构体中添加 `status_panel` 字段 — 挂载面板状态
  - 位置: `peri-tui/src/app/mod.rs:75-103` (App 结构体定义)
  ...
  - 位置: `peri-tui/src/app/mod.rs` 顶部模块声明区（L1-11），在 `pub mod agent_panel;` 之后追加:

    ```rust
    pub mod status_panel;
    ```

  - 位置: `peri-tui/src/app/mod.rs` re-export 区（L66-71），追加:
...
- [x] 创建 `/cost` 命令 — 打开 StatusPanel 并激活 Cost Tab
  - 新建: `peri-tui/src/command/cost.rs`
  - 内容:
...
- [x] 创建 `/context` 命令 — 打开 StatusPanel 并激活 Context Tab
  - 新建: `peri-tui/src/command/context_cmd.rs`
  - 内容:
...
- [x] 注册 cost 和 context 命令到默认注册表
  - 位置: `peri-tui/src/command/mod.rs`（L1-10 模块声明区）
  ...
  - 位置: `peri-tui/src/command/mod.rs:14-25` (`default_registry` 函数)
...
- [x] 创建 StatusPanel 渲染模块 — 实现 Cost 和 Context 两个 Tab 的只读展示
  - 新建: `peri-tui/src/ui/main_ui/panels/status.rs`
  - 内容:
    ...
    use peri_widgets::{BorderedPanel, tab_bar::TabBar};
...
- [x] 注册面板渲染模块 — 在 panels/mod.rs 中声明 status 模块
  - 位置: `peri-tui/src/ui/main_ui/panels/mod.rs:1-7`
  - 在文件末尾追加:
...
- [x] 在主渲染函数中分发 StatusPanel 渲染 — 添加面板到渲染管线
  - 位置: `peri-tui/src/ui/main_ui.rs:94-99`（cron 和 mcp 面板渲染之后）
...
- [x] 在 `active_panel_height` 中添加 StatusPanel 高度计算 — 固定 14 行（标签栏 1 + 空行 1 + 内容 10 + 边框 2）
  - 位置: `peri-tui/src/ui/main_ui.rs:126-208` (`active_panel_height` 函数)
...
- [x] 在事件分发链中添加 StatusPanel 优先处理 — 面板激活时拦截按键
  - 位置: `peri-tui/src/event.rs` 面板分发区域（~L184-206，model_panel 检查之后）
...
- [x] 在状态栏快捷键提示中添加 StatusPanel 分支 — 显示 ←→:切换Tab Esc:关闭
  - 位置: `peri-tui/src/ui/main_ui/status_bar.rs:225-273` (`render_second_row` 的 `None` 分支)
...
- [x] 为 StatusPanel 编写单元测试
  ...
  - 运行命令: `cargo test -p peri-tui --lib -- status_panel`
  - 预期: 所有测试通过

**检查步骤:**

- [x] 验证新增文件存在且结构正确
  - `ls peri-tui/src/app/status_panel.rs peri-tui/src/ui/main_ui/panels/status.rs peri-tui/src/command/cost.rs peri-tui/src/command/context_cmd.rs`
  - 预期: 4 个文件均存在
- [x] 验证编译通过
  - `cargo build -p peri-tui 2>&1 | tail -5`
  - 预期: 输出包含 "Finished" 且无 error
- [x] 验证命令注册正确
  - `grep -n 'cost::CostCommand\|context_cmd::ContextCommand' peri-tui/src/command/mod.rs`
  - 预期: 两行注册语句存在
- [x] 验证 AgentComm 新字段存在
  - `grep -n 'session_start_time\|tool_call_count' peri-tui/src/app/agent_comm.rs`
  - 预期: 结构体定义和 Default 初始化各出现 2 次
- [x] 验证面板渲染分发存在
  - `grep -n 'status_panel\|render_status_panel' peri-tui/src/ui/main_ui.rs`
  - 预期: 渲染分支和高度计算分支均存在
- [x] 验证事件处理链存在
  - `grep -n 'handle_status_panel' peri-tui/src/event.rs`
  - 预期: 函数定义和调用各 1 处
- [x] 验证测试通过
  - `cargo test -p peri-tui --lib -- status_panel 2>&1 | tail -10`
  - 预期: 所有测试通过
...
- 新建: `peri-tui/src/app/memory_panel.rs`（核心）
- 新建: `peri-tui/src/ui/main_ui/panels/memory.rs`（核心）
- 新建: `peri-tui/src/command/memory.rs`（核心）
- 注: app/mod.rs/command/mod.rs/panel_ops.rs 的集成修改由 Task 5 统一执行

**执行步骤:**

- [x] 创建 MemoryPanel 状态结构体 — 管理面板的条目列表、光标和滚动偏移
  - 新建文件: `peri-tui/src/app/memory_panel.rs`
...
- [x] 在 app/mod.rs 注册 memory_panel 模块并添加 App 字段
  - 位置: `peri-tui/src/app/mod.rs` L1-10 的 pub mod 区域，在 `pub mod login_panel;` 之后添加 `pub mod memory_panel;`
  - 位置: `peri-tui/src/app/mod.rs` re-export 区（~L66-71），追加:
  ...
  - 位置: `peri-tui/src/app/mod.rs:75-103` (App 结构体定义)，在 `pub status_panel: Option<status_panel::StatusPanel>,` 之后追加:
...
- [x] 创建 MemoryCommand — `/memory` 命令实现
  - 新建文件: `peri-tui/src/command/memory.rs`
  - 内容:
...
- [x] 注册 MemoryCommand 到 default_registry
  - 位置: `peri-tui/src/command/mod.rs` L1-10 的 pub mod 区域，在 `pub mod mcp;` 之后添加:

    ```rust
    pub mod memory;
    ```

  - 位置: `peri-tui/src/command/mod.rs:14-25` (`default_registry` 函数)，在 `r.register(Box::new(mcp::McpCommand));` 之后追加:
...
- [x] 在 App 中添加 MemoryPanel 操作方法 — 光标移动、打开编辑器、关闭
  - 位置: `peri-tui/src/app/panel_ops.rs`，在 CronPanel 操作区域之后添加 MemoryPanel 操作区域
...
- [x] 创建 MemoryPanel 渲染模块 — 实现 2 条目列表展示
  - 新建文件: `peri-tui/src/ui/main_ui/panels/memory.rs`
...
- [x] 为 MemoryPanel 状态逻辑编写单元测试
  - 测试文件: `peri-tui/src/app/memory_panel.rs` 的 `#[cfg(test)] mod tests` 块
  - 测试场景:
  ...
  - 运行命令: `cargo test -p peri-tui --lib -- memory_panel::tests`
  - 预期: 所有测试通过
...
- [x] 为 MemoryPanel 渲染和命令注册编写 headless 集成测试
  ...
  - 运行命令: `cargo test -p peri-tui --lib -- memory`
  - 预期: 所有测试通过

**检查步骤:**

- [x] 验证新增文件存在且结构正确
  - `ls peri-tui/src/app/memory_panel.rs peri-tui/src/ui/main_ui/panels/memory.rs peri-tui/src/command/memory.rs`
  - 预期: 3 个文件均存在
...
- [x] 验证命令注册正确
  - `grep -n 'memory::MemoryCommand' peri-tui/src/command/mod.rs`
  - 预期: 注册行存在

- [x] 验证 App 结构体包含 memory_panel 字段
  - `grep -n 'memory_panel' peri-tui/src/app/mod.rs`
  - 预期: 模块声明、re-export、App 字段、new() 初始化共 4 处

- [x] 验证 panel_ops 包含 memory_panel 操作方法
  - `grep -n 'memory_panel_' peri-tui/src/app/panel_ops.rs`
  - 预期: move_up/move_down/open_editor/close 共 4 个方法

- [x] 验证测试通过
  - `cargo test -p peri-tui --lib -- memory_panel::tests 2>&1 | tail -10`
  - 预期: 所有 3 个单元测试通过
...
- 修改: `peri-tui/src/event.rs` — 添加 config/status/memory 面板事件处理函数和分发拦截
- 修改: `peri-tui/src/ui/main_ui/status_bar.rs` — 添加新面板快捷键提示分支
- 修改: `peri-tui/src/ui/main_ui.rs` — 添加面板渲染分发和高度计算
- 修改: `peri-tui/src/app/panel_ops.rs` — 添加新面板 open/close/apply 操作方法
- 修改: `peri-tui/src/app/mod.rs` — 添加模块声明和 re-export
- 修改: `peri-tui/src/ui/main_ui/panels/mod.rs` — 添加模块声明
- 修改: `peri-tui/src/app/core.rs` — 添加 config_panel 字段（如 Task 2 未在此文件操作）
- 修改: `peri-tui/src/app/agent_comm.rs` — 添加 session_start_time 和 tool_call_count 字段（如 Task 3 未在此文件操作）
- 修改: `peri-tui/src/app/agent_ops.rs` — session_start_time 赋值和 tool_call_count 递增（如 Task 3 未在此文件操作）

**执行步骤:**

- [x] 在 `app/mod.rs` 中声明新面板模块 — 注册 Task 2/3/4 创建的模块
  - 位置: `peri-tui/src/app/mod.rs` L1-10 的 pub mod 区域
...
- [x] 在 `app/mod.rs` 中添加 re-export — 供外部模块直接使用
  - 位置: `peri-tui/src/app/mod.rs` L66-71 的 re-export 区域
...
- [x] 在 `App` 结构体中添加新面板字段 — 挂载面板状态
  - 位置: `peri-tui/src/app/mod.rs` L75-103 (App 结构体)
...
- [x] 在 `App::new()` 和 `new_headless()` 初始化中添加新字段
  - 位置: `peri-tui/src/app/mod.rs` `App::new()` 返回值初始化（~L164-196）
...
- [x] 在 `submit_message` 中初始化 `session_start_time` — 首次提交时记录时间
  - 位置: `peri-tui/src/app/agent_ops.rs` `submit_message` 方法（~L21-48）
...
- [x] 在 `panel_ops.rs` 中添加 ConfigPanel 操作方法
  - 位置: `peri-tui/src/app/panel_ops.rs`，在 Login 面板操作区域之后（~L173）
  - 添加 Config 面板操作区域:
...
- [x] 在 `panel_ops.rs` 中添加 StatusPanel 操作方法
  - 位置: `peri-tui/src/app/panel_ops.rs`，在 Config 面板操作之后
  - 添加 Status 面板操作区域:
...
- [x] 在 `panel_ops.rs` 中添加 MemoryPanel 操作方法
  - 位置: `peri-tui/src/app/panel_ops.rs`，在 Status 面板操作之后
  - 添加 Memory 面板操作区域:
...
- [x] 在 `event.rs` 中添加 ConfigPanel 事件处理函数
  - 位置: `peri-tui/src/event.rs`，在 `handle_model_panel` 函数之后（~L1134）
  - 追加:
...
- [x] 在 `event.rs` 中添加 MemoryPanel 事件处理函数
  - 位置: `peri-tui/src/event.rs`，在 `handle_config_panel` 函数之后
  - 追加:
...
- [x] 在 `event.rs` 中添加 StatusPanel 事件处理函数（如 Task 3 未完成此操作）
  - 位置: `peri-tui/src/event.rs`，在 `handle_memory_panel` 函数之后
  - 追加:
...
- [x] 在 `event.rs` 的事件分发链中插入新面板拦截 — 按优先级顺序
  - 位置: `peri-tui/src/event.rs` `next_event` 函数，面板分发区域（~L172-206）
...
- [x] 在 `Event::Paste` 分支中添加新面板拦截 — 防止粘贴文本穿透到 textarea
  - 位置: `peri-tui/src/event.rs` `Event::Paste` 分支（~L529-564）
...
- [x] 在 `main_ui.rs` 渲染分发中添加新面板渲染 — model 面板之后
  - 位置: `peri-tui/src/ui/main_ui.rs:70-100`，面板渲染条件链
...
- [x] 在 `status_bar.rs` 的 `render_second_row` 中添加新面板快捷键提示
  - 位置: `peri-tui/src/ui/main_ui/status_bar.rs:225-273` (`render_second_row` 的 `None` 分支内部条件链)
...
- [x] 更新 `/cost` 和 `/context` 命令使用 panel_ops 方法 — 确保互斥逻辑一致
  - 位置: `peri-tui/src/command/cost.rs`（如 Task 3 创建时直接赋值 `app.status_panel`）
  - 将 `execute` 方法中的直接赋值:
  ...
  - 位置: `peri-tui/src/command/context_cmd.rs`，同样替换为 `app.open_status_panel(STATUS_TAB_CONTEXT);`
...
- [x] 为集成编写 headless 端到端测试 — 验证事件分发 + 渲染 + 互斥
  ...
  - 运行命令: `cargo test -p peri-tui --lib -- headless::integration`
  - 预期: 所有测试通过

**检查步骤:**

- [x] 验证模块声明完整
  - `grep -n 'config_panel\|status_panel\|memory_panel' peri-tui/src/app/mod.rs`
  - 预期: 模块声明、re-export、App 字段各出现对应条目

- [x] 验证事件分发链包含新面板
  - `grep -n 'handle_config_panel\|handle_status_panel\|handle_memory_panel' peri-tui/src/event.rs`
  - 预期: 每个函数有定义和调用各 1 处

- [x] 验证渲染分发包含新面板
  - `grep -n 'config_panel\|status_panel\|memory_panel\|render_config\|render_status\|render_memory' peri-tui/src/ui/main_ui.rs`
  - 预期: 渲染分支和高度计算分支均存在

- [x] 验证状态栏快捷键提示完整
  - `grep -n 'config_panel\|status_panel\|memory_panel' peri-tui/src/ui/main_ui/status_bar.rs`
  - 预期: 三个面板各有快捷键提示分支

- [x] 验证 panel_ops 包含所有新方法
  - `grep -n 'open_config_panel\|open_status_panel\|open_memory_panel\|close_config_panel\|close_status_panel\|close_memory_panel\|config_panel_apply' peri-tui/src/app/panel_ops.rs`
  - 预期: 所有方法均存在
...
- [x] 验证 agent_ops 正确更新新字段
  - `grep -n 'session_start_time\|tool_call_count' peri-tui/src/app/agent_ops.rs`
