...
**自动验证点:**
- [x] [A] $schema 字段序列化/反序列化 roundtrip: `cargo test -p rust-agent-tui -- config::types 2>&1` → 期望包含: `test result: ok`
...
**自动验证点:**
- [x] [A] excludes 匹配时 CLAUDE.md 被跳过 + 不匹配时正常加载: `cargo test -p rust-agent-middlewares -- agents_md 2>&1` → 期望包含: `test result: ok`
  - **来源:** spec-plan.md Task 1 检查步骤
  - **目的:** 验证 glob 排除逻辑正确性
...
**自动验证点:**
- [x] [A] CLAUDE.local.md 合并逻辑全场景: `cargo test -p rust-agent-middlewares -- agents_md 2>&1` → 期望包含: `test result: ok`
...
**自动验证点:**
- [x] [A] @import 解析全场景（简单/嵌套/超深/循环/不存在/非法格式）: `cargo test -p rust-agent-middlewares -- agents_md 2>&1` → 期望包含: `test result: ok`
...
**自动验证点:**
- [x] [A] /effort 命令编译与测试: `cargo test -p rust-agent-tui -- effort 2>&1` → 期望包含: `test result: ok`
...
**自动验证点:**
- [x] [A] update_title 持久化与 updated_at 更新: `cargo test -p rust-create-agent -- sqlite_store 2>&1` → 期望包含: `test result: ok`
  - **来源:** spec-plan.md Task 4 检查步骤
  - **目的:** 验证 SQL 更新和时间戳
...
**自动验证点:**
- [x] [A] /doctor 命令编译与测试: `cargo test -p rust-agent-tui -- doctor 2>&1` → 期望包含: `test result: ok`
...
- [x] [A] 全量构建无 warning: `cargo build 2>&1 | grep -c warning` → 期望精确: `0`
  - **来源:** spec-plan.md Task 6
  - **目的:** 验证代码质量
- [x] [A] TUI crate 编译通过: `cargo build -p rust-agent-tui 2>&1 | tail -3` → 期望包含: `Finished`
