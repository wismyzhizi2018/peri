...
- 配置层（Task 1-3）全部修改 `rust-agent-middlewares/src/agents_md.rs` 和 `rust-agent-tui/src/config/types.rs`，按 C2+C6 → C1 → C4 顺序实施
- TUI 命令层（Task 4-5）在 `rust-agent-tui/src/command/` 下新增 3 个命令文件，修改 `mod.rs` 注册
...
- 修改: `rust-agent-middlewares/src/agents_md.rs`
- 修改: `rust-agent-tui/src/app/agent.rs`

**执行步骤:**
- [x] 在 PeriConfig 新增 `$schema` 字段 — passthrough，不影响逻辑
  - 位置: `rust-agent-tui/src/config/types.rs` PeriConfig struct（~L7）
...
- [x] 在 AppConfig 新增 `claude_md_excludes` 字段
  - 位置: `rust-agent-tui/src/config/types.rs` AppConfig struct（~L131，`extra` 字段之前）
  - 插入:
    ```rust
...
- [x] 在 AgentsMdMiddleware 新增 `excludes` 字段和 `with_excludes()` builder
  - 位置: `rust-agent-middlewares/src/agents_md.rs` struct 定义（~L18-20）
...
- [x] 修改 agent.rs 传入 excludes
  - 位置: `rust-agent-tui/src/app/agent.rs`（~L284）
...
- [x] 为新增字段和排除逻辑编写单元测试
  - 测试文件: `rust-agent-tui/src/config/types.rs` tests 模块 + `rust-agent-middlewares/src/agents_md.rs` tests 模块
  - 场景:
    - `$schema` 序列化/反序列化 roundtrip → 保留且不影响逻辑
    - `claude_md_excludes` 为 None 时序列化不输出该字段
    - excludes 匹配时 CLAUDE.md 被跳过
    - excludes 不匹配时正常加载
  - 运行: `cargo test -p rust-agent-middlewares -- agents_md && cargo test -p rust-agent-tui -- config::types`
  - 预期: 全部通过
...
- [x] PeriConfig 新字段编译通过
  ...
  - 预期: `Finished` 无 error
- [x] 排除逻辑测试通过
  - `cargo test -p rust-agent-middlewares -- agents_md 2>&1 | tail -5`
  - 预期: 所有 test ok
...
- [x] 为 CLAUDE.local.md 读取编写单元测试
  - 测试文件: `rust-agent-middlewares/src/agents_md.rs` tests 模块
  - 场景:
  ...
  - 运行: `cargo test -p rust-agent-middlewares -- agents_md`
  - 预期: 全部通过
...
- [x] @import 单元测试通过
  ...
  - 预期: 所有 test ok
- [x] 编译无 warning
  - `cargo build -p rust-agent-middlewares 2>&1 | grep -c warning`
  - 预期: 输出 0
...
**涉及文件:**
- 新建: `rust-agent-tui/src/command/effort.rs`
- 新建: `rust-agent-tui/src/command/rename.rs`
...
- [x] 在 ThreadStore trait 新增 `update_title()` 方法
  - 位置: `rust-create-agent/src/thread/store.rs`（~L34，`delete_thread` 之后）
  - 添加默认方法:
    ```rust
    /// 更新指定 thread 的标题
...
- [x] 在 SqliteThreadStore 新增 `update_title()` 优化实现
  - 位置: `rust-create-agent/src/thread/sqlite_store.rs`（~L249，`update_meta` 之后）
  - 添加:
    ```rust
    async fn update_title(&self, id: &ThreadId, title: &str) -> Result<()> {
...
- [x] 为 update_title 编写单元测试
  - 测试文件: `rust-create-agent/src/thread/sqlite_store.rs` tests 模块
  - 场景:
    - update_title 后 load_meta 返回新标题
    - update_title 后 updated_at 已更新
  - 运行: `cargo test -p rust-create-agent -- sqlite_store`
  - 预期: 全部通过
...
- [x] 命令编译通过
  ...
  - 预期: `Finished` 无 error
- [x] update_title 测试通过
  - `cargo test -p rust-create-agent -- sqlite_store 2>&1 | tail -5`
  - 预期: 所有 test ok
...
**涉及文件:**
- 新建: `rust-agent-tui/src/command/doctor.rs`
- 修改: `rust-agent-tui/src/command/mod.rs`
...
- [x] 为 doctor 命令编写 headless 集成测试（可选，简单场景）
  - 测试文件: `rust-agent-tui/src/command/doctor.rs` 底部 `#[cfg(test)] mod tests`
  - 场景:
    - 无配置时输出包含 "No Provider" 和 "Missing"
  - 运行: `cargo test -p rust-agent-tui -- doctor`
  - 预期: 通过
...
- [x] 命令编译通过
  ...
  - 预期: `Finished` 无 error
- [x] doctor 测试通过
  - `cargo test -p rust-agent-tui -- doctor 2>&1 | tail -5`
  - 预期: 所有 test ok
...
**前置条件:**
- 启动命令: `cargo build -p rust-agent-tui && cargo test`
...
2. - [x] C2 验证: `$schema` 字段 passthrough
   ...
   - `cargo test -p rust-agent-tui -- config::types 2>&1 | tail -5`
   - 预期: 所有 test ok
...
5. - [x] T1 验证: /effort 命令切换
   - `cargo test -p rust-agent-tui -- effort 2>&1 | tail -5`
   - 预期: 测试通过
