...
**版本**：v7（v6 修 9 处 bug：§5.2.2 view 层 PartialEq 描述错误、§1.1 事实 F 副本对应错位 + 测量噪声 + 空行、§5.2.3 冷路径 trade-off、§5.2.4 计数偏差、§6.5 测试代码缺 tool result/compact 处理、§4.1 方案 B 数字无依据）
...
**副本数对应说明**：§1.2 实际涉及 ToolResult 大字符串 deep clone 的路径为 #2 origin_messages、#4 completed_tools.output、#5 ToolBlock.content、#7 RenderCache.lines 共 **4 个 deep clone**，加上 #1 原始 = 5 份。mock 取 7 份（6 clone + 1 原始）是**上限估算**，覆盖 view 层潜在冗余（如 Phase 2 才修的 #6 ContentBlockView::Text.raw）。7 份 vs 5 份的偏差对结论无影响——节省比例理论值 (N-1)/N 在 N=7 时为 85.7%，N=5 时为 80%，均与实测 92-100% 吻合。
...
**测试方法说明**：mock 用 `/proc/self/status` VmRSS 测量，每次测量前 `force_alloc()` 触发分配器归还，黑箱消费避免编译器消除。**Arc<str> RSS 显示 0 KB 是 VmRSS 测量精度限制**：Arc<str> 的 7 份引用实际占 7 × (8 字节栈指针 + 16 字节堆 ref counter header) ≈ 168 字节 + 1 份原始字符串，总开销远低于 VmRSS 4 KB 页粒度，故显示 0。issue 报告的端到端测试用的是 `estimate_messages_heap` 精确堆估算，二者在 7 轮 3MB 场景下的结果一致（误差 < 25%），互相印证。
...
| B. + View 层 Arc<str>（Phase 2） | 4 | #2、#4、#5、#6（注：#6 不属于 ToolResult 路径，详见 §5.3） | ~10-15 MB（Phase 1 ~15-20 MB 基础上减 #5 的 5MB） | Phase 0 实测后填入 | 中 | ✅（更宽松） |
...
| `as_text()` 调用 | ≥10 文件（grep `as_text()` 范围内调用，精确数字未实测） | 返回类型 `Option<&str>` 不变，**零改动** |
...
| `MessageContent::Text(s)` pattern matching 后 `.len()` / `.is_empty()` / `json!(s)` | 7 处（gc.rs:197 + openai.rs:13 + anthropic.rs:77,210 + invoke.rs:48,81,142） | Deref 或 Serialize 透明，**零改动** |
...
fn feed_round(
    ...
    user_text: &str,
    ai_msg: BaseMessage,
    tool_results: Vec<BaseMessage>, // 5% 大文件场景注入 500KB-3MB ToolResult；其它轮次空 Vec
) {
    pipeline.begin_round();
    // 流式 chunks（模拟 AssistantChunk）
    // 若 tool_results 非空：先发 ToolUse 消息，再发对应 ToolResult 消息（按 §2.2 比例分布）
    // StateSnapshot 携带 user + ai 消息
    // done
}

// 在 measure_500_rounds_typical_baseline 主循环内必须额外处理 compact 事件：
//   match event {
//       AgentEvent::CompactCompleted(messages) => {
//           pipeline.clear();
//           pipeline.restore_completed(messages);
//           // 同步 view_messages：调用 pipeline.messages_to_view_models() 重建
//           // 这是 §1.1 事实 E 描述的 TUI 层清理路径
//       }
//       _ => { /* ... */ }
//   }
// 注意：500 轮预期触发 8-15 次 full_compact，未处理会导致 RSS 曲线持续上涨、与真实 TUI 行为不符。
...
| v6 bug | v7 修正 |
|---------|---------|
| **§5.2.2** 提到 `MessageViewModel` 手动 PartialEq 需"小改"——但 Phase 1 不改 view 层字段（§5.1 明确），自相矛盾 | 改为"Phase 1 全部不动，该 PartialEq 无需任何改动"，view 层 PartialEq 改造推到 Phase 2 |
| **§1.1 事实 F** 说"模拟 6 个存储点路径（每轮 6 副本）"，但 §1.2 实际只 4 deep clone + 1 原始 = 5 份，mock 用 7 份（6 clone + 1 原始），对应数错位 | 新增"副本数对应说明"段：mock 是上限估算（覆盖 #6 view 层冗余），7 份 vs 5 份对结论无影响，理论节省 80% vs 85.7% 均与实测 92-100% 吻合 |
| **§1.1 事实 F** Arc<str> RSS 显示 0 KB 未解释，与关键观察 #2（168 字节开销）自相矛盾 | 补充测量精度说明：Arc<str> 7 份引用 ≈ 168 字节，远低于 VmRSS 4 KB 页粒度 |
| **§1.1 事实 F** 后缺空行，markdown 渲染粘连 | 补空行 |
| **§5.2.3** sqlite_store/filesystem 修复 `t.clone()` → `t.to_string()` 未说明 trade-off | 新增"冷路径 trade-off 说明"：每次 set_completed 一次性 memcpy，不在 hot path；Phase 1 在该路径不保留 Arc 共享，但整体收益不受影响（hot path 在 TUI 层 origin_messages.clone() / completed_tools.output） |
| **§5.2.4** "Deref/Serialize 透明兼容 6 处"实际 7 处（invoke.rs:81,142 一行算 2 处） | 改为 7 处，并在表中列出完整行号清单 |
| **§5.2.4** "as_text() 调用 13 文件"无 grep 实测命令 | 改为"≥10 文件（精确数字未实测）"，避免误导 |
| **§6.5** `feed_round(pipeline, round_idx, user_text, ai_msg)` 不支持注入大 ToolResult | 函数签名新增 `tool_results: Vec<BaseMessage>` 参数，5% 大文件场景通过此参数注入 |
| **§6.5** 测试代码未处理 compact 事件 | 新增 `AgentEvent::CompactCompleted` 处理伪代码，说明 500 轮预期触发 8-15 次 compact，未处理会导致 RSS 曲线失真 |
| **§4.1** 方案 B "5MB 单轮 ~10-12 MB"无依据 | 改为 ~10-15 MB，注明"Phase 1 ~15-20 MB 基础上减 #5 的 5MB" |
