...
**位置**：`rust-create-agent/src/llm/anthropic.rs:412-418`
...
| 文件 | 位置 | 说明 |
|------|------|------|
| `rust-create-agent/src/agent/events.rs:18` | `AiReasoning(String)` 定义 | 事件已定义但从未 emit |
| `rust-agent-tui/src/app/message_pipeline.rs:195-206` | `AiReasoning` 分支 | 接收处理逻辑完整但永远不会执行 |
| `rust-agent-tui/src/app/message_pipeline.rs:334-336` | `push_reasoning()` | 追加到 `current_ai_reasoning`，但此缓冲区始终为空 |
| `rust-agent-tui/src/app/message_pipeline.rs:538-539` | `has_streaming_content()` | 检查 `current_ai_reasoning`，始终 false |
| `rust-agent-tui/src/app/message_pipeline.rs:563-566` | `build_streaming_bubble()` | 构建 `ContentBlockView::Reasoning`，永远不会执行 |
| `rust-agent-tui/src/ui/message_view.rs:447` | `ContentBlockView::Reasoning` | 视图变体定义，流式路径不可达 |
...
- `rust-create-agent/src/llm/anthropic.rs:402-428` — extended thinking 占位 thinking 注入逻辑
- `rust-create-agent/src/llm/openai.rs:646-650` — OpenAI generate_reasoning 提取 thought（仅 Text blocks，正确）
- `rust-create-agent/src/agent/executor/tool_dispatch.rs:29-44` — tool_dispatch 明确选择 TextChunk 而非 AiReasoning
- `rust-create-agent/src/agent/executor/final_answer.rs:76-89` — final_answer 同样选择 TextChunk
- `rust-create-agent/src/agent/events.rs:18` — AiReasoning 事件定义
- `rust-agent-tui/src/app/message_pipeline.rs:195-206,334-336,538-539,563-566` — TUI 接收链
- `rust-agent-tui/src/ui/message_view.rs:447` — Reasoning 视图变体
